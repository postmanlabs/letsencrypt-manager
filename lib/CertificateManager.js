            ...
            async.auto({
                ...
                order: ['nonce', 'url', 'key', (results, next) => {
                    ...
                    (err, response, data) => {
                        ...
                        if (data.status !== 'pending') {
                            return next(new LetsEncryptError(`Invalid State! Expeted pending, got ${data.status}`));
                        }
                ...
                verifyChallenge: ['challenge', (results, next) => {
                    ...
                    async.retry({
                        times: 5,
                        interval: (count) => {
                            return 100 + 100 * Math.pow(2, count);
                        },
                        errorFilter: (err) => {
                    ...
                    (cb) => {
                        ...
                        (err, response, data) => {
                            ...
                            if (data.status === 'pending' || data.status === 'processing') {
                                return cb(new LetsEncryptError('Pending'));
                            }
                    ...
                    (cb) => {
                        ...
                        (err, response, data) => {
                            ...
                            if (data.status === 'processing') {
                                return cb(new LetsEncryptError('Processing'));
                            }
