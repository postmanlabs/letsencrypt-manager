const _ = require('lodash'),
    ...
    Order = require('./Order'),
    { generateKey, formatKey, generateCSR } = require('./util/keys'),
...
class CertificateManager {
    ...
    _getKeypair (domain, email, done) {
        this.manager.store.certificates.checkKeypair([domain], (err, keypair) => {
            ...
            generateKey((keygenErr, generatedKey) => {
                if (keygenErr) { return done(keygenErr); }

                let keypair = formatKey(generatedKey);
